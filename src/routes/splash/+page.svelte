...
<div class="size-full flex flex-col items-center relative">
    ...
    <div class="size-full flex flex-col justify-between p-6">
        <div class="space-y-3">
            <img src={images.logo} class="w-10" alt="Logo" />
            <h1 class="text-2xl font-semibold">{info.name}</h1>
        </div>
        <p class="animate-pulse" in:fade={{ duration: 400, delay: 400 }}>{states[status]()}</p>
    </div>
    <img src={images.meshLight} class="fixed top-0 bottom-0 right-0 block dark:hidden -z-1" alt="{info.name} Mesh" />
    <img src={images.meshDark} class="fixed top-0 bottom-0 right-0 hidden dark:block -z-1" alt="{info.name} Mesh" />
</div>