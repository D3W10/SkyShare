...
<div use:outsideClick on:outclick={() => { if (open) open = !open; }}>
    <Button type="invisible" className="block relative {!open ? "z-10" : "z-20"} {className}" {disabled} on:click={() => open = !open}>
        ...
        {#if open}
            <div class="w-full absolute {!listReverse ? "top-6" : "bottom-6"} bg-secondary rounded-md overflow-hidden -z-10" transition:slide={$transition.comboFlow}>
                <div id="comboboxItems" class="max-h-[7.5rem] flex flex-col bg-foreground/10 overflow-y-scroll {listClassName}">
                    {#each items as item, i}
                        <Button type="invisible" className="px-2 py-1.5 text-left text-sm {i == 0 && !listReverse ? "pt-4" : (i == items.length - 1 && listReverse ? "pb-3.5" : "")}" on:click={() => itemSelected(i)}>{item}</Button>
                    {/each}
                </div>
            </div>
        {/if}
