<script lang="ts">
    ...
    $app?.updateCallback("uri", uriHandler);
    $app?.updateCallback("login", (username, password) => account.login(username, password, true));
    $app?.updateCallback("error", (code: number) => error.set(code));
